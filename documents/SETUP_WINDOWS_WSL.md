# Windows with WSL2 host setup

This guide sets up a Docker Jenkins controller and the Linux image for Android and Web on Windows with WSL2. iOS uses a separate native Mac agent. Follow this guide once per host, then use [Configuration](CONFIGURATION.md) for jobs and [Operations](OPERATIONS.md) for troubleshooting, backups and validation evidence.

Other hosts: [Linux](SETUP_LINUX.md), [macOS](SETUP_MACOS.md).

- [Requirements and preparation](#prepare)
- [Install host prerequisites](#windows)
...
Start with 4+ CPU cores, 12-16 GB RAM available to Docker and 100 GB free SSD for one Docker build at a time. Windows needs additional OS memory (16 GB total was the original starting recommendation). These are sizing recommendations, not guaranteed minima. Caches, image layers and archived artifacts grow over time. Serialize heavy builds.
...
<a id="windows"></a>
## Windows: install WSL2 and Docker Desktop

The recipe was written for Windows 10 (2004+) or Windows 11 on x86-64 with virtualization enabled in BIOS/UEFI. Check [Docker's current Windows requirements](https://docs.docker.com/desktop/setup/install/windows-install/) for your selected installer.
...
Restart if requested and finish Ubuntu's first-run username/password setup. Keep an existing Ubuntu installation. If its listed version is 1:

```powershell
wsl --set-version Ubuntu 2
```

Install/start [Docker Desktop for Windows](https://docs.docker.com/desktop/setup/install/windows-install/). Enable **Use WSL 2 based engine**, **Linux containers**, and Ubuntu under **Settings > Resources > WSL Integration**; apply and restart. The tray menu offering "Switch to Windows containers" indicates the current mode is Linux. Review memory/disk limits for the WSL/Docker backend and leave Windows headroom. See [Docker's WSL integration instructions](https://docs.docker.com/desktop/features/wsl/).

Do not also install Docker Engine inside integrated Ubuntu; this path uses Desktop's daemon. Open Ubuntu:
...
If `docker` is unavailable, check Desktop is running and Ubuntu integration is enabled. Remaining commands run in Ubuntu Bash. Desktop must remain running; configure sleep/restarts for CI availability. This recipe does not install an unattended Windows service. Continue with [the central repository checkout](#checkout).
...
Run in the Ubuntu WSL terminal. Use a stable directory for the complete central checkout, including hidden files:
...
Replace the placeholder URL; use an existing checkout if already present. On WSL prefer `~/ci` over `/mnt/c/...` for filesystem performance and permissions. No host/CI files go into Flutter repositories. [The host manifest](../manifests/host.csv) lists host deliverables/checksums; preserve `.env.example`, `.dockerignore` and other hidden files when transferring the folder.
...
In Ubuntu WSL, run the read-only preflight from `host/`:
